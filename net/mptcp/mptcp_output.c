...
//				TCP_SKB_CB(orig_skb)->mptcp_rbs_flags_not_in_queue, TCP_SKB_CB(skb)->mptcp_rbs_flags_not_in_queue);
	 ...
	 */
	mptcp_find_and_set_pathmask(meta_sk, skb); // why is this needed?
...
void mptcp_send_fin(struct sock *meta_sk)
{
	...
	if(tcp_send_head(meta_sk) != NULL && skb == NULL) {
	...
	if (tcp_send_head(meta_sk) != NULL &&
		skb != NULL /* afr: ist das wirklich ein fix?  ohne das hier gibt es manchmal am Ende einen absturz, weil der send_head auf etwas zeigt, aber die queue ler ist.*/) {
