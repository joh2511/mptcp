				   ...
				   struct mptcp_rbs_eval_ctx *ctx)
{
	...
	val = self->left_operand->execute(self->left_operand, ctx);
	if (val == -1)
		return -1;
	result = val;
				      ...
				      struct mptcp_rbs_eval_ctx *ctx)
{
    ...
    lval = self->left_operand->execute(self->left_operand, ctx);
	if (lval == -1)
		return -1;

	rval = self->right_operand->execute(self->right_operand, ctx);
	if (rval == -1 || !rval)
		return -1;

    remainder = do_div(lval, rval);
...
u64 mptcp_rbs_sbf_get_bw_send(struct mptcp_rbs_sbf_cb *sbf_cb)
{
    ...
    result = (RBS_BW_INTERVAL_NS - diff) * sbf_cb->bw_out_bytes;
    do_div(result, RBS_BW_INTERVAL_NS);
	return result;
}
...
u64 mptcp_rbs_sbf_get_bw_ack(struct mptcp_rbs_sbf_cb *sbf_cb)
{
	...
	/* only use the portion of one second */
    result = (RBS_BW_INTERVAL_NS - diff) * sbf_cb->bw_ack_bytes;
    do_div(result, RBS_BW_INTERVAL_NS);
	return result;
}
			  ...
			  unsigned int bytes)
{
	...
	if (ct - *last_update_ns <
        ...
        result = (RBS_BW_INTERVAL_NS - delta_t) * (*bytes_in_cb);
        do_div(result, RBS_BW_INTERVAL_NS);
		*bytes_in_cb = result + bytes;
